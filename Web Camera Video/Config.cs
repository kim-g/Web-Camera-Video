  ...
  public class Config
  {
    ...
    public void SaveToFile()
    {
      using (Stream stream = (Stream) new FileStream("config.xml", FileMode.Create))
        new XmlSerializer(typeof (Config)).Serialize(stream, (object) this);
    }
    ...
    public static Config LoadFromFile()
    {
      ...
      {
        Config config1 = (Config) new XmlSerializer(typeof (Config)).Deserialize(stream);
